/**
 * Seed script for Hays HS Christmas Classic - Lehman Pool
 * This tournament demonstrates multiple game advancement with losers feeding into a consolation pool
 * Run with: npx ts-node scripts/seed-lehman-pool.ts
 */
...
async function seedLehmanPoolTournament() {
  console.log('🏀 Creating Hays HS Christmas Classic - Lehman Pool...\n');
  ...
  try {
    ...
    const locations = [
      { name: 'Blue Gym', address: 'Lehman High School', city: 'Kyle', state: 'TX' },
    ];
    ...
    const championshipBracket = {
      ...
      seeds: [
        { position: 1, teamName: 'Lehman' },
        { position: 2, teamName: 'Crockett' },
        { position: 3, teamName: 'St. John Paul II' },
        { position: 4, teamName: 'Stafford' },
        { position: 5, teamName: 'San Marcos' },
        { position: 6, teamName: 'Austin Prep' },
        { position: 7, teamName: 'McNeil' },
        { position: 8, teamName: 'Austin Royals' },
      ],
      createdAt: Timestamp.now(),
      updatedAt: Timestamp.now(),
    };
    ...
    const round1Game1 = await upsertGame({
      ...
      locationId: locationIds['Blue Gym'],
      court: 'Blue Gym',
      status: 'scheduled',
    ...
    console.log('📊 Summary:');
    console.log('   - 1 tournament: Hays HS Christmas Classic - Lehman Pool');
    console.log('   - 1 division: Varsity Boys - Lehman Pool');
    console.log('   - 1 location: Blue Gym');
